...
@interface ORDatabaseModel ()
@property (strong, nonatomic) NSPersistentStoreCoordinator *persistentStoreCoordinator;
@end

@implementation ORDatabaseModel
...
+(id)getClassInstance{
    static ORDatabaseModel* databaseModel=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        databaseModel=[[ORDatabaseModel alloc]init];
    });
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [applicationDocumentsDirectory URLByAppendingPathComponent:@"ConverterLab.sqlite"];
...
-(void)saveContext{
    ...
    if (![self.managedObjectContext save:&error]) {
        ...
        // Show Alert View
        [[[UIAlertView alloc] initWithTitle:@"Warning" message:@"Database could not be saved." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
    }
}

#pragma mark -
#pragma mark get stores supporting methods (compatible & incompatible stores)
...
-(void)showWarning{
    ...
    NSString *applicationName = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"ORBundleDisplayName"];
